...
public class ExporterBlockEntityRenderer implements BlockEntityRenderer<ExporterBlockEntity, ExporterBlockEntityRendererState> {
    ...
    public void extractRenderState(ExporterBlockEntity blockEntity, ExporterBlockEntityRendererState state, float partialTicks, Vec3 cameraPosition, ModelFeatureRenderer.CrumblingOverlay breakProgress) {
        ...
        state.exporterPosition = new GlobalPos(blockEntity.getLevel().dimension(), blockEntity.getBlockPos());
        state.exporterFacing = blockEntity.getLevel()
                .getBlockState(blockEntity.getBlockPos())
                .getValue(ExporterBlock.FACING);
        state.importerPositions = blockEntity.importerPositions;
    }
    ...
    public void submit(ExporterBlockEntityRendererState state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
        ...
        List<GlobalPos> importerPositions = state.importerPositions;
        Vec3 camera = cameraRenderState.pos;

        Vec3 exporterPos = getBeamStart(state.exporterPosition.pos(), level);

        ExporterBlockEntity be = (ExporterBlockEntity) Objects.requireNonNull(level.getBlockEntity(state.exporterPosition.pos()));
        InputItemHandler itemHandler = be.getUpgradeItemHandler();
        ...
        for (GlobalPos importerPosRaw : importerPositions) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                submitNodeCollector.submitCustomGeometry(poseStack, NO_CULL_BEAM, (pose, consumer) -> {
                    ...
                    local.translate((float) beamStartWorld.x, (float) beamStartWorld.y, (float) beamStartWorld.z);
                    Quaternionf rotation = new Quaternionf().rotationTo(
                            new Vector3f(0, 1, 0),
                            new Vector3f((float) beamDir.x, (float) beamDir.y, (float) beamDir.z)
                    );
                    ...
                    for (int face = 0; face < 4; face++) {
                        ...
                        addBeamVertex(consumer, facePose, -beamRadius, (float) beamLength, -beamRadius, r, g, b, a, 0f, vOffset + (float) beamLength, 0xF000F0, OverlayTexture.NO_OVERLAY);
                        addBeamVertex(consumer, facePose,  beamRadius, (float) beamLength, -beamRadius, r, g, b, a, 1f, vOffset + (float) beamLength, 0xF000F0, OverlayTexture.NO_OVERLAY);
                        addBeamVertex(consumer, facePose,  beamRadius, 0f,                 -beamRadius, r, g, b, a, 1f, vOffset,                      0xF000F0, OverlayTexture.NO_OVERLAY);
                        addBeamVertex(consumer, facePose, -beamRadius, 0f,                 -beamRadius, r, g, b, a, 0f, vOffset,                      0xF000F0, OverlayTexture.NO_OVERLAY);
    ...
    private float[] getBeamTint(ItemStack stack) {
        ...
        for (ButtonType type : RouterButtonTypes.BUTTONS.values()) {
            if (stack.is(type.getUnlockedBy())) {
